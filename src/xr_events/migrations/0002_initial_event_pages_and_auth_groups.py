...
def add_initial_event_pages_and_auth_groups(apps, schema_editor):
    ...
    event_list_page_path = ("%12d" % (int(last_child_path) + 1)).replace(" ", "0")
    event_list_page, created = EventListPage.objects.get_or_create(
    ...
    event_group_page_path = "%s0001" % event_list_page_path
    event_group_page, created = EventGroupPage.objects.get_or_create(
        title="XR Deutschland",
        is_regional_group=True,
        slug="xr_de",
        content_type=event_group_content_type,
        path=event_group_page_path,
        depth=4,
        numchild=0,
